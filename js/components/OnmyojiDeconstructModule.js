// OnmyojiDeconstructModule - 阴阳师模块拆解组件
const { useState, useEffect } = React;

const OnmyojiDeconstructModule = ({ item }) => {
  ...
  useEffect(() => {
    // 直接使用全局数据
    if (typeof ONMYOJI_DOC_FIXED !== 'undefined') {
      setDocumentData(ONMYOJI_DOC_FIXED);
      setLoading(false);
    } else {
      // 如果全局数据不可用，尝试加载
      fetch('js/data/onmyoji-doc-fixed.js')
        .then(response => response.text())
        .then(text => {
          // 提取数据
          const match = text.match(/window\.ONMYOJI_DOC_FIXED = ({[\s\S]*?});/);
          if (match) {
  ...
  if (viewMode === 'original') {
    return (
      <div className="min-h-screen bg-[#fdfcf8]">
        ...
        {/* iframe 加载原始HTML */}
        <iframe
          src="assets/阴阳师模块拆解/阴阳师手游-部分模块详细拆解.htm"
